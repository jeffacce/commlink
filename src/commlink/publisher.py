...
class Publisher:
    def __init__(
        ...
        legacy_serializer: if True, use standard pickle.dumps (compatible with older commlinks).
                           if False (default), use pickle protocol 5 + multipart out-of-band buffers.
        compression: optional codec for payload compression. One of None, 'zstd', 'lz4'.
                     Cannot be combined with legacy_serializer=True. The wire format is
                     self-describing, so subscribers do not need a matching setting.
        ...
        """
        if legacy_serializer and compression is not None:
            raise ValueError("compression is not supported with legacy_serializer=True")
